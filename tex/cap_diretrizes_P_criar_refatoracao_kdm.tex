...
\multicolumn{1}{|c|}{\textbf{<\%Arg1\%>}} & Nome do módulo. Pode-se utilizar qualquer nome válido na linguagem ATL; \\ 
\hline
...
\begin{codigo}[caption={[\textit{Template} ATL para realizar a operação atômica \textit{add}.] \textit{Template} ATL para realizar a operação atômica \textit{add}.},escapeinside={(*@}{@*)}, basicstyle=\footnotesize, label={codigo:template_addKDMElement}, language=ATL]{Name}
module add(*@<\textbf{\%Arg1\%}>@*);
...
%
no Código-fonte~\ref{codigo:exemplo_add_classUnit} é apresentado uma simples ATL que adiciona uma nova instância da metaclasse \texttt{ClassUnit} denominada \texttt{\aspas{novaClassUnit}} no pacote \texttt{com.br.teste}. Como pode ser observado na linha 5 é especificado o nome do pacote (\texttt{\aspas{com.br.teste}}) que é onde a nova instância da \texttt{ClassUnit} será adicionada. A linha 8 representa que a nova instância da \texttt{ClassUnit} será adicionada no associação \texttt{codeElement}. Nas linhas 10-12 uma nova instância da metaclasse \texttt{ClassUnit} é adicionada e seu nome definido como \aspas{novaClassUnit}.
