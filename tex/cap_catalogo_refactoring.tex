...
No contexto desta Tese todas as refatorações são transformações que são realizadas em um instância do metamodelo KDM. Assim, todas as refatorações podem ser agrupadas em nível de sua granularidade. A granularidades podem ser agrupas em dois níveis de operações: (\textit{i}) operações atômicas ou (\textit{ii}) operações compostas. Mais especificadamente as granularidades de operações atômicas podem ser definidas por meio de operações primitivas que são executadas no metamodelo KDM. Tais operações primitivas são listadas a seguir:
